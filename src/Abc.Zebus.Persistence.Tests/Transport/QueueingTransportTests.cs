    ...
    public class QueueingTransportTests
    {
        ...
        public void should_send_PersistenceStopping_Message_and_wait_for_acks_before_stopping()
        {
            ...
            using (MessageId.PauseIdGeneration())
            {
                ...
                _innerTransport.ExpectExactly(new TransportMessageSent(new TransportMessage(MessageTypeId.PersistenceStopping, Stream.Null, self), targets));

                _innerTransport.RaiseMessageReceived(new TransportMessage(MessageTypeId.PersistenceStoppingAck, Stream.Null, _allPeers[0].Peer));
                _innerTransport.RaiseMessageReceived(new TransportMessage(MessageTypeId.PersistenceStoppingAck, Stream.Null, _allPeers[1].Peer));
