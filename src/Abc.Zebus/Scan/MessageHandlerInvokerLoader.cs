...
public abstract class MessageHandlerInvokerLoader : IMessageHandlerInvokerLoader
{
    ...
    private static bool IsCustomInvokerMessageHandlerInterface(Type interfaceType)
        => interfaceType.IsGenericType && interfaceType.GetGenericTypeDefinition() == typeof(ICustomInvokerMessageHandler<>) && !interfaceType.GetGenericArguments()[0].IsGenericParameter;

    private bool IsMessageHandlerInterface(Type interfaceType)
        => interfaceType.IsGenericType && interfaceType.GetGenericTypeDefinition() == _genericHandlerType && !interfaceType.GetGenericArguments()[0].IsGenericParameter;
}
