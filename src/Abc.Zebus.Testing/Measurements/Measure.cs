...
namespace Abc.Zebus.Testing.Measurements
{
    ...
    {
        private const double _탎InOneSecond = 1000000;
        private static readonly object _lock = new object();
        private static readonly double _탎Frequency = _탎InOneSecond / Stopwatch.Frequency;
        ...
        private static void PrintResults(MeasureConfiguration configuration, BenchResults results)
        {
            ...
            lock (_lock)
            {
                ...
                Console.WriteLine("Min :          {0,10:### ### ##0}탎", min * _탎Frequency);
                Console.WriteLine("Avg :          {0,10:### ### ##0}탎", (double)results.Elapsed.Ticks / configuration.Iteration / (TimeSpan.TicksPerMillisecond / 1000));
                Console.WriteLine("Median :       {0,10:### ### ##0}탎", median * _탎Frequency);
                Console.WriteLine("95 percentile : {0,10:### ### ##0}탎", fivePercentile * _탎Frequency);
                Console.WriteLine("99 percentile : {0,10:### ### ##0}탎", onePercentile * _탎Frequency);
                Console.WriteLine("Max :          {0,10:### ### ##0}탎 (Iteration #{1})", max * _탎Frequency, results.MaxIterationIndex);
